...
# 运行命令
# nohup python ~/ipynb/isp.py >> ~/isp.output_overexpress_N2CD.log 2>&1 &
...
embs_output_directory = "~/geneformer_testdata/isp/output_embs"
...
for indexgene, gene_pair in enumerate(tqdm(gene_combinations, desc="Processing gene combinations")):
    gene_pair = list(gene_pair)  # Convert gene_pair to a list here
...
for gene_pair in tqdm(valid_combinations, desc="Processing gene pairs", position=0, leave=True):
    try:
        perturb_type = "overexpress"  # {"delete", "overexpress", "inhibit", "activate"}
